...
import { createValue, getDefaultComparison, getDefaultTextValue, ignoreCaseCompare } from '@/util/functions';
...
export const FieldSelection = React.memo(() => {
  ...
  const filteredFields = React.useMemo(() => fields.filter(f => ignoreCaseCompare(f.title, filter)), [fields, filter]);
  ...
  const handleAddMatcher = useDynamicCallback((event: React.MouseEvent, field: Field) => {
    ...
    addValue(matcher, editPosition, AND, getDefaultComparison(field));
    setShowFields(false);
    setFilter('');
    event.stopPropagation();
  });

  const handleChanged = useDynamicCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    setFilter(event.currentTarget.value);
  });

  const handleKeyDown = useDynamicCallback((event: React.KeyboardEvent) => {
    if (event.key === 'Enter' && filteredFields.length > 0) {
      handleAddMatcher(filteredFields[0]);
    }
  });
    ...
    <div className={s.fieldSelection}>
      ...
      {showFields &&
        <div className={s.filterSelectionPopup} onMouseLeave={handleHideFields}>
          <div className={s.filter}><input value={filter} onChange={handleChanged} onKeyDown={handleKeyDown} /></div>
          <div className={s.fields}>
            {filteredFields.map(f => <div className={s.field} key={f.name} onClick={(e) => handleAddMatcher(e, f)}>{f.title}</div>)}
          </div>
        </div>}
    </div>)
});