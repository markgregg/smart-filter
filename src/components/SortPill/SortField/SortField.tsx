...
export const SortField = React.memo(
  ({ sort }: SortFieldProps) => {
    ...
    const display = React.useMemo(() => {
      if (fieldRef.current) {
        const parentLeft = fieldRef.current.parentElement?.offsetLeft ?? fieldRef.current.offsetLeft;
        const right = fieldRef.current.offsetLeft - parentLeft + fieldRef.current.offsetWidth;
