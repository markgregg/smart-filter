...
): UseBoundStore<StoreApi<MatcherState>> => {
  const { fields, matchers: managedMatchers, onChange, onClear } = props;
  ...
  return create<MatcherState>((set) => {
    const setNotify = (
      updateState:
        ...
        | ((state: MatcherState) => MatcherState | Partial<MatcherState>),
